...
GwikiUI.prototype.init = function () {
    ...
    // Add event listeners
    // Errors
    this.gwiki.addEventListener('error', function (e) {
        t.block(e.message);
    });
    this.bridge.addEventListener('error', function (e) {
        t.block(e.message);
    });

    // Information updates
    this.gwiki.addEventListener('setHome', function (e) {
        t.onSetHome(e);
    });
    this.gwiki.addEventListener('setCurrentItem', function (e) {
        t.onSetCurrentItem(e);
    });
    this.gwiki.addEventListener('rectifyTree', function (e) {
        t.drawSideMenu(e);t.updateMenuSelections(e);
    });
    this.gwiki.addEventListener('updateNavStack', function (e) {
        t.drawBreadcrumbs(e);
    });

    // Signin status
    this.bridge.addEventListener('signinStatusChanged', function (e) {
        t.toggleSignedIn();
    });
    ...
    if (this.config.prettyUrls) {
        ...
        if (path != '/') {
            ...
            if (path.length == '2') {
                ...
                if (this.defaultHome) doc = path[0];else home = path[0];
            }
        }
...
GwikiUI.prototype.onSetHome = function (e) {
    if (e.target.home) this.home = e.target.home.id;else this.home = null;
...
GwikiUI.prototype.onSetCurrentItem = function (e) {
    if (e.target.currentItem) this.doc = e.target.currentItem.id;else this.doc = false;
    this.updateState(this.home, this.doc);
...
GwikiUI.prototype.askForHome = function () {
    ...
    blocker.getElementsByTagName('form')[0].addEventListener('submit', function (e) {
        ...
        if (!folderId) alert('Invalid Id! Please enter either the full url from a google drive folder, or just the id part.');else {
            t.loading(true);
...
GwikiUI.prototype.toggleSignedIn = function () {
    ...
    if (this.bridge.signedIn) {
        this.unblock();
        if (!this.gwiki.home) {
            if (this.home) this.gwiki.setHome(this.home, this.doc);else this.askForHome();
        } else {
            this.drawHeader();
            if (this.gwiki.currentItem.id == this.doc) this.drawMainContent();else this.gwiki.setCurrentItem(this.doc);
        }
    } else {
        ...
        btn.addEventListener('click', function (e) {
            t.loading(true);t.bridge.signin();
        });
    }
};
...
GwikiUI.prototype.drawMainContent = function () {
    ...
    if (!this.gwiki.currentItem.bodySrc) {
        ...
        if (this.gwiki.currentItem.bodySrc.gwikiType == 'text/markdown') this.mainContent.innerHTML = this.parseMarkdown(this.gwiki.currentItem.body);else if (this.gwiki.currentItem.bodySrc.gwikiType == 'text/html') this.mainContent.innerHTML = this.cleanHtml(this.gwiki.currentItem.body);else {
...
GwikiUI.prototype.updateMenuSelections = function () {
    ...
    for (var i = 0; i < this.mainMenu.length; i++) {
        if (selected && this.mainMenu[i].gobject.id == selected.id) this.mainMenu[i].classList.add('selected');else this.mainMenu[i].classList.remove('selected');
    }
...
GwikiUI.prototype.buildHierarchicalMenu = function (node) {
    ...
    if (selected && node.children && node.children.length > 0) {
        for (var i = 0; i < node.children.length; i++) {
            container.appendChild(this.buildHierarchicalMenu(node.children[i]));
        }
    }
...
GwikiUI.prototype.getSelectedSubtree = function () {
    var selected;
    if (this.gwiki.currentItem.isHome) selected = null;else {
        selected = this.gwiki.currentItem;
        while (selected.parents && !selected.parents[0].isHome) {
            selected = selected.parents[0];
        }
    }
...
GwikiUI.prototype.loading = function (loading) {
    if (loading) this.block(GwikiUI.strings.displayTitle + GwikiUI.strings.tagline + GwikiUI.strings.loading);else this.unblock();
};
...
GwikiUI.prototype.updateState = function (home, doc) {
    ...
    if (title.length > 0) title = title.join(' - ');else title = GwikiUI.strings.title;
    ...
    if (this.config.prettyUrls) {
        ...
        if (this.defaultHome) {
            ...
            if (this.home) query.home = this.home;else delete query.home;
        }

        // Add doc, or delete if not available
        if (this.doc) query.doc = this.doc;else delete query.doc;
...
GwikiUI.prototype.parseMarkdown = function (md) {
    if (!(this.markdownParser instanceof Markdown.Converter)) {
        if (typeof this.markdownParser == 'function') this.markdownParser.call(this);else throw "this.markdownParser must be either an instance of Markdown.Converter or a container function that yields a Markdown.Converter";
    }
