...
GwikiItem = function (_GwikiItem) {
    function GwikiItem(_x, _x2) {
        return _GwikiItem.apply(this, arguments);
    }

    GwikiItem.toString = function () {
        return _GwikiItem.toString();
    };
...
GwikiItem.prototype.setExtraAttributes = function () {
    ...
    if (this.isConsumable === null && this.mimeType && this.name) {
        this.isConsumable = GwikiItem.consumableTypes.indexOf(this.mimeType) > -1 || this.name.substr(-3) == '.md';
    }

    // See if it's text format
    if (this.gwikiType === null && this.mimeType && this.name) {
        if (this.mimeType == 'text/x-markdown' || this.mimeType == 'text/markdown' || this.mimeType == 'text/plain' || this.name.substr(-3) == '.md') this.gwikiType = 'text/markdown';else if (this.mimeType == 'text/html' || this.mimeType == 'application/vnd.google-apps.document') this.gwikiType = 'text/html';else this.gwikiType = 'unknown';
    }
...
GwikiItem.prototype.getParents = function () {
    ...
    if (returnCounter == t.parents.length) return go;else return thenGo;
};
...
GwikiItem.cache = {};
GwikiItem.consumableTypes = ['application/vnd.google-apps.folder', 'application/vnd.google-apps.document', 'application/vnd.google-apps.spreadsheet', 'application/vnd.google-apps.presentation', 'text/x-markdown', 'text/markdown', 'text/html', 'application/rtf', 'application/vnd.oasis.opendocument.text', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.oasis.opendocument.presentation', 'application/vnd.openxmlformats-officedocument.presentationml.presentation', 'application/pdf', 'text/plain'];
